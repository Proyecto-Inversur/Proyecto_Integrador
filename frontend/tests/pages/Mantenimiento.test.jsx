...
describe('Componente Mantenimiento', () => {
  ...
  // Prueba para verificar el renderizado con un tipo no usuario
  test('renderiza solo los botones generales para un tipo no usuario', () => {
    const authContextValue = {
    ...
    // Verificamos que el contenedor se renderice
    const container = screen.getByText(/Mantenimiento Correctivo/i).closest('.home-container');
    expect(container).toBeInTheDocument();

    // Verificamos que solo se rendericen los botones generales
    expect(screen.queryByText(/Cuadrillas/i)).toBeNull();
    expect(screen.queryByText(/Sucursales/i)).toBeNull();
    expect(screen.queryByText(/Preventivos/i)).toBeNull();

    // Verificamos los botones generales y sus rutas
    const generalButtons = [
      { text: 'Mantenimiento Correctivo', route: '/mantenimientos-correctivos' },
      { text: 'Mantenimiento Preventivo', route: '/mantenimientos-preventivos' },
    ];

    generalButtons.forEach(({ text, route }) => {
      const button = screen.getByText(text);
      expect(button).toBeInTheDocument();
      expect(button.closest('a')).toHaveAttribute('href', route);
    });
